--[[
	...
	LucianosHub Interface Suite
	by Luciano Ferretti

	Luciano Ferretti is not responsible for any damage caused by this software. Use at your own risk.

]]

if debugX then
	warn('Initialising LucianosHub')
end
...
local requestsDisabled = true -- Set to true to disable all remote requests, including analytics and interface loading. This is for users who want to use the interface but are concerned about privacy or security implications of remote requests.
local InterfaceBuild = 'LHv2'
local Release = "Build 1.68"
local LucianosHubFolder = "LucianosHub"
local ConfigurationFolder = LucianosHubFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		lucianoshubdOpen = {Type = 'bind', Value = 'K', Name = 'LucianosHub Keybind'},
		-- buildwarnings
		-- lucianoshubprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.lucianoshubOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("LucianosHub | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, LucianosHubFolder) then
				if callSafely(isfile, LucianosHubFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, LucianosHubFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"lucianoshubOpen":{"Value":"K","Type":"bind","Name":"LucianosHub Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"LucianosHub Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
			...
			-- If no settings saved, apply overridden settings only
				...
				for settingName, settingValue in pairs(overriddenSettings) do
					local split = string.split(settingName, ".")
					assert(#split == 2, "LucianosHub | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('LucianosHub had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "LucianosHub")
...
local LucianosHub = useStudio and script.Parent:FindFirstChild('LucianosHub') or game:GetObjects("rbxassetid://89151290560548")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local lucianoshubDestroyed = false -- True when LucianosHubLibrary:Destroy() is called

repeat
	if LucianosHub:FindFirstChild('Build') and LucianosHub.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('LucianosHub | Build Mismatch')
		print('LucianosHub may encounter issues as you are running an incompatible interface version ('.. ((LucianosHub:FindFirstChild('Build') and LucianosHub.Build.Value) or 'No Build') ..').\n\nThis version of LucianosHub is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
toDestroy, LucianosHub = LucianosHub, useStudio and script.Parent:FindFirstChild('LucianosHub') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

LucianosHub.Enabled = false

if gethui then
	LucianosHub.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(LucianosHub)
	LucianosHub.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	LucianosHub.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == LucianosHub.Name and Interface ~= LucianosHub then
			Interface.Enabled = false
			Interface.Name = "LucianosHub-Old"
		end
	end
elseif not useStudio then
...
if LucianosHub.AbsoluteSize.X < minSize.X and LucianosHub.AbsoluteSize.Y < minSize.Y then
...
local Main = LucianosHub.Main
local MPrompt = LucianosHub:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = LucianosHub:FindFirstChild('Drag')
...
local Notifications = LucianosHub.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when LucianosHub is destroyed

local SelectedTheme = LucianosHubLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = LucianosHubLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	LucianosHub.Main.BackgroundColor3 = SelectedTheme.Background
	LucianosHub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	LucianosHub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	LucianosHub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	LucianosHub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	LucianosHub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	LucianosHub.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		LucianosHub.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		LucianosHub.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(LucianosHub:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("LucianosHub | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("LucianosHub | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('LucianosHub had an issue decoding the configuration file, please try delete the file and reopen LucianosHub.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(LucianosHubLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("LucianosHub | Unable to find '"..FlagName.. "' in the save file.")
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(LucianosHubLibrary.Flags) do
...
function LucianosHubLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('LucianosHub | Not seeing your actions in notifications?')
			print("Notification Actions are being sunset for now, keep up to date on when they're back in the discord. (https://discord.gg/jMk4zkDZXV)")
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			LucianosHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			LucianosHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "lucianoshubOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, LucianosHubFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('LucianosHub Settings', 0, true)

	if TabList['LucianosHub Settings'] then
		TabList['LucianosHub Settings'].LayoutOrder = 1000
	end

	if Elements['LucianosHub Settings'] then
		Elements['LucianosHub Settings'].LayoutOrder = 1000
	end
...
function LucianosHubLibrary:CreateWindow(Settings)
	if LucianosHub:FindFirstChild('Loading') then
		if getgenv and not getgenv().lucianoshubCached then
			LucianosHub.Enabled = true
			LucianosHub.Loading.Visible = true

			task.wait(1.4)
			LucianosHub.Loading.Visible = false
		end
	end

	if getgenv then getgenv().lucianoshubCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				LucianosHubLibrary:Notify({Title = 'Build Mismatch', Content = 'LucianosHub may encounter issues as you are running an incompatible interface version ('.. ((LucianosHub:FindFirstChild('Build') and LucianosHub.Build.Value) or 'No Build') ..').\n\nThis version of LucianosHub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "LucianosHub"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "LucianosHub Interface Suite" then
		LoadingFrame.Version.Text = "LucianosHub UI"
	end
	...
	if not Settings.DisableLucianosHubPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				LucianosHubLibrary:Notify({
					Title = "LucianosHub Interface",
					Content = "Enjoying this UI library? Find it at https://discord.gg/jMk4zkDZXV",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(LucianosHubFolder.."/Discord Invites")

		if callSafely(isfile, LucianosHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, LucianosHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"LucianosHub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(LucianosHubFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("LucianosHub | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if callSafely(isfile, LucianosHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, LucianosHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, LucianosHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						LucianosHubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called LucianosHubLibrary:Destroy()
				if lucianoshubDestroyed then
					return
				end
				if not Success then
					...
					print("LucianosHub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= LucianosHubLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					LucianosHubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			LucianosHub.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("LucianosHub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					LucianosHubLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("LucianosHub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					LucianosHubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("LucianosHub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					LucianosHubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= LucianosHubLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("LucianosHub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						LucianosHubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("LucianosHub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					LucianosHubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			LucianosHubLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			LucianosHubLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('LucianosHub had an issue creating settings.') end
...
function LucianosHubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function LucianosHubLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function LucianosHubLibrary:Destroy()
	lucianoshubDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['LucianosHub Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "lucianoshubOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function LucianosHubLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				LucianosHubLibrary:Notify({Title = "LucianosHub Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			LucianosHubLibrary:Notify({Title = "LucianosHub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('LucianosHub Configurations Error | '..tostring(result))
			LucianosHubLibrary:Notify({Title = "LucianosHub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	LucianosHubLibrary.LoadConfiguration()
